...
// 23. Imprimir os nós por nível
void imprimirNivel(No *raiz, int nivelAtual, int nivelAlvo) {
    if (raiz == NULL) return;
    ...
    imprimirNivel(raiz->esq, nivelAtual + 1, nivelAlvo); // Desce para os filhos
    if (nivelAtual > 0) {
        imprimirNivel(raiz->dir, nivelAtual, nivelAlvo);  // Anda para os irmãos
    }
}

// 25. Destruir/Liberar toda a árvore da memória
